// 作品一覧ページ(新作 / 予約)の共通レンダラ。
// /products?type=... はクエリ付き＝robots.txt で拒否しているURLなので、
// 索引対象は /new と /pre-order（このモジュールを直接呼ぶ）。
import { NextRequest, NextResponse } from 'next/server';
import { readHtml } from './readHtml';
import { injectMobileLayout, injectWebLayout } from './injectLayout';
...
/** 新作一覧(/new)のSEOメタ */
export const NEW_PRODUCTS_SEO: HubMeta = {
    title: 'AV新作一覧',
    description: 'FANZA・MGSのAV新作を発売日順に毎日更新。メーカー・シリーズ・出演女優から絞り込み、両プラットフォームの価格を比べて選べます。',
    path: '/new',
    breadcrumb: '新作',
};

/** 予約一覧(/pre-order)のSEOメタ */
export const PREORDER_PRODUCTS_SEO: HubMeta = {
    title: 'AV予約・近日発売作品',
    description: 'FANZA・MGSで予約受付中のAV作品を発売日順に掲載。話題作の発売予定をいち早くチェックして予約できます。',
    path: '/pre-order',
    breadcrumb: '予約',
};
...
(function(){
  ...
  // /pre-order と /new はクエリ無しで開かれる(旧: /products?type=...への302)ので path からも判定する
  var type=params.get('type')||(location.pathname.indexOf('/pre-order')===0?'pre-order':'new');
...
export async function renderProductsPage(request: NextRequest, forcedType?: string, seo?: HubMeta) {
    const ua = request.headers.get('user-agent') || '';
    const isMobile = MOBILE_UA.test(ua);
    const params = new URL(request.url).searchParams;
    const type = forcedType || params.get('type') || 'new';

    const htmlFile = isMobile
        ? '/design/products.html'
        : `/design/web/${type === 'pre-order' ? 'pre-order.html' : 'new-products.html'}`;

    // 索引対象の /new・/pre-order は再クロールをエッジで返す(UAでHTMLが変わるので m/w を鍵に含める)
    const edge = seo ? await edgeLookup(request.url, isMobile ? 'm' : 'w') : null;
    ...
    try {
        ...
        // /new・/pre-order のときだけ <title>(products.html は空だった)/description/canonical を入れる。
        // H1 も共用テンプレの「作品一覧」のままだったのでページ固有の見出しにする。
        if (seo) {
        ...
        const resp = new NextResponse(html, {
            ...
            headers: {
                ...
                'Cache-Control': seo
                    ? 'public, s-maxage=1800, max-age=60'
                    : 'private, max-age=60',
                ...(seo ? { 'CDN-Cache-Control': 'public, s-maxage=1800' } : {}),
            },
        });
        if (edge) await edgeStore(edge, resp);
        return resp;
    } catch {
