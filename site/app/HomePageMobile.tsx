    ...
    return (
        <Link href={`/product/${encodeURIComponent(p.product_id)}`} className="min-w-[110px] w-[110px] shrink-0 block group">
            <div className="aspect-[2/3] rounded-lg bg-slate-200 mb-2 overflow-hidden shadow-sm">
                {poster && (
                    <img alt="" className="w-full h-full object-cover object-right group-hover:scale-105 transition-transform" src={poster} loading="lazy"
    ...
    return (
        <Link href={`/product/${encodeURIComponent(p.product_id)}`} className="min-w-[260px] p-3 bg-white rounded-xl border border-primary/10 shadow-sm flex gap-3 shrink-0 block group">
            <div className="w-16 aspect-square rounded-lg overflow-hidden shrink-0 bg-slate-100">
                {p.main_image_url && (
                    <img alt="" className="w-full h-full object-cover object-right group-hover:scale-105 transition-transform" src={getPosterImageUrl(p.main_image_url)} loading="lazy" />
                )}
            </div>
...
export default function HomePageMobile() {
    ...
    useEffect(() => {
        ...
        fetch('/api/products?sort=pre-order&source=fanza&limit=6' + mkParam)
            .then(r => r.json())
            .then(setPreorder)
            .catch(() => {});
        // 注目・ランキング
        ...
        fetch(`/api/products?sort=new&source=fanza&fromDate=${today}&toDate=${today}&limit=10` + mkParam)
            .then(r => r.json())
            .then(data => {
                if (data && data.length > 0) { setNewArrivals(data); return; }
                const threeDaysAgo = fmt(new Date(Date.now() - 3 * 86400000));
                return fetch(`/api/products?sort=new&source=fanza&fromDate=${threeDaysAgo}&toDate=${today}&limit=10` + mkParam)
                    .then(r => r.json())
                    .then(setNewArrivals);
            })
            .catch(() => {});
