...
function parseDetailPage(html) {
    ...
    const actress_links = [];
    let sale_start_date = '';
    // 商品発売日（元の発売日）。旧作を MGS で再配信すると「配信開始日」だけが新しくなるので、
    // 2016年の作品が2026年の新作に見えていた（906KAGH-045: 配信開始日 2026/09/13・商品発売日 2016/01/25）。
    let release_date = '';
    ...
    $('table tr').each((_, row) => {
        ...
        } else if (th.includes('商品発売日')) {
            // 「DVD未発売」等の文言は捨てて日付だけ持つ
            const m = td.text().trim().match(/\d{4}\/\d{1,2}\/\d{1,2}/);
            release_date = m ? m[0] : '';
    ...
    return { title, actresses: actresses.join(', '), actress_links, maker, label, duration_min, wish_count, genres: genres.join(', '), sale_start_date, release_date, list_price, current_price, discount_pct, sale_end_date };
}
